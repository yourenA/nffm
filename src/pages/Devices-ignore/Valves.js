...
import { Alert,Badge,InputNumber,Select,Card,Button,List ,message,Collapse,Switch ,Form ,Icon,Tabs,Row,Col  } from 'antd';
          ...
          <Tabs onChange={this.onChangeTabs} style={{backgroundColor:'#fff'}} activeKey={this.state.activeKey}>
            {
              double_ball_valves.data.map((item,index)=>{
                ...
                if(this.state.currentValve.increment_valve){
                  switch (this.state.currentValve.increment_valve.current_status){
                    case -1:
                ...
                if(this.state.currentValve.decrement_valve){
                  switch (this.state.currentValve.decrement_valve.current_status){
                    case -1:
                    ...
                    >
                      <div style={{marginBottom:'6px'}}>
                        <DescriptionList  size="small" col="4" >
                          <Description term="是否已使用"> <h3>{this.state.currentValve.is_enabled===1?'是':'否'}</h3></Description>
                          <Description term="模式"> <h3>{this.state.currentValve.is_enabled===1?this.state.currentValve.mode===0?'手动模式':'恒压控制':'未知'}</h3></Description>
                        </DescriptionList>
                      </div>
                      {
                      ...
                      <div style={{marginBottom:'10px'}}>
                        <DescriptionList  size="small" col="4" >
                          <Description term="增压球阀名称">  {this.state.currentValve.increment_valve&&this.state.currentValve.increment_valve.name}</Description>
                          <Description term="增压球阀状态">  {increment_valve_current_status}</Description>
                        </DescriptionList>
                        <DescriptionList  size="small" col="4" >
                          <Description term="减压球阀名称">  {this.state.currentValve.decrement_valve&&this.state.currentValve.decrement_valve.name}</Description>
                          <Description term="减压球阀状态">  {decrement_valve_current_status}</Description>
                        </DescriptionList>
                        </div>
                        <div style={{marginBottom:'10px'}}>
                        <DescriptionList  size="small" col="1" >
                          <Description term="恒压控制可选传感器">  {this.state.currentValve.optional_sensors&&this.state.currentValve.optional_sensors.reduce((pre,item)=>{
                            pre.push(item.name);
                            return pre
                          },[]).join(' | ')}</Description>
                        </DescriptionList>
                          ...
                          <Row gutter={16}>
                            <Col span={6}>
                              <Card className={styles.valve_card} size="small" title={'增压球阀'}>
                                <h4>目标状态 :<Switch className="valve-switch" checked={this.state[item.id]&&this.state[item.id].increment_valve_status} checkedChildren="开" unCheckedChildren="关"  onChange={(checked)=>this.handleChangeValve(item.id,'increment_valve_status',checked)}/></h4>
                              </Card>
                            </Col>
                            <Col span={6}>
                              <Card className={styles.valve_card} size="small" title={'减压球阀'}>
                                <h4>目标状态 :<Switch className="valve-switch" checked={this.state[item.id]&&this.state[item.id].decrement_valve_status} checkedChildren="开" unCheckedChildren="关"  onChange={(checked)=>this.handleChangeValve(item.id,'decrement_valve_status',checked)}/></h4>
                              </Card>
                            </Col>
                            </Row>
